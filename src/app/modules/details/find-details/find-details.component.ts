...
export class FindDetailsComponent implements OnInit{
  ...
  constructor(
    public dialogRef: MatDialogRef<FindDetailsComponent>,
    ...
    try {
      ...
      if (error) {
        ...
        this.clinicForm.patchValue({
          ...
          cEmail: this.clinic[0]?.cEmail || '',
          cNumber: this.clinic[0]?.cNumber || '',
          cTime: this.clinic[0]?.cTime || '',
          cSchedule: this.clinic[0]?.cSchedule || '',
          cService: this.clinic[0]?.cService || '',
          cGrmprice: this.clinic[0]?.cGrmprice || '',
          cLabPrice: this.clinic[0]?.cLabPrice || '',
          cSrgPrice: this.clinic[0]?.cSrgPrice || '',
          cLink: this.clinic[0]?.cLink || '',
          distance: this.clinic[0]?.distance || '',
        })
      }
    } catch (error) {
  ...
  formatDistance(distance: number | undefined): string {
    if (distance !== undefined && distance !== null) {
      return `${distance.toFixed(2)} km`; // Format distance with two decimal places and append "km"
    } else {
      return ''; // Return empty string if distance is null or undefined
    }
  }
  ...
  async fetchClinicLogo(){
    const filename = `${this.clinic.cName}_clinicLogo`;
    ...
    try{
      ...
      if(error){
        this.clinic.profile = 'assets/logoo.png'
      } else{
        this.clinic.profile = URL.createObjectURL(data);
      }
    } catch(error){
      this.clinic.profile = 'assets/logoo.png'
    }
