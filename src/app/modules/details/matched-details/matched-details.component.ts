...
import { MakeMatchedAppointmentComponent } from '../../make-matched-appointment/make-matched-appointment.component';
...
@Component({
  selector: 'app-matched-details',
  templateUrl: './matched-details.component.html',
  styleUrls: ['./matched-details.component.scss']
})
export class MatchedDetailsComponent implements OnInit{
  clinics: any[] = [];
  ...
  //               .select('*')
  //               .eq('mClinic_id', clinicId);
  //               console.log('load matched',clinicsData)
  
  //           if (clinicsError) {
  //             console.error('Error fetching matched clinics:', clinicsError);
  //           } else {
  //             this.clinic = clinicsData;
  //             console.log('intial clinic',this.initialClinic)
  //           }
  //         } else {
  //           console.error('No user data found for the logged-in user.');
  //         }
  ...
  async popSeeMoreDetails(clinic: any) {
    const dialogRef = this.dialog.open(MatchedDetailsComponent, {
      width: '50%',
      height: 'auto',
      data: { clinic: clinic },
    });
